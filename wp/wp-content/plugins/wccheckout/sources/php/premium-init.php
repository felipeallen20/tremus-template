...
add_action( 'cfw_updated_to_1032', array( TelemetryCollection::instance(), 'run_telemetry_update' ) );
			...
			add_action(
				...
				function ( $user_id ) {
					if ( function_exists( 'as_schedule_single_action' ) ) {
						as_schedule_single_action( time() + 300, 'cfw_link_orders_for_customer', array( $user_id ), 'checkoutwc' );
						wc_get_logger()->info( "CheckoutWC: Scheduled order linking for user {$user_id} in 5 minutes", array( 'source' => 'checkout-wc' ) );

						return;
					}

					wc_get_logger()->warning( 'CheckoutWC: Action Scheduler not available, user matching will not work', array( 'source' => 'checkout-wc' ) );
				}
			);
